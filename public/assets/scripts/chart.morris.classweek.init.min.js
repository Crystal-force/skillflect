...
let class_weekchart = $("#class_weekid").val();
...
$.ajax({
  ...
  success:function(data){
    ...
    let calue;
      if(count == '0'){
        $("#week_notification").delay(5).fadeIn('slow').delay(1500).fadeOut('slow');
      }
      else{
  ...
  function initChart_ClassWeek() {
    r = Morris.Donut({
      element: "donut-morris-chart-classweek",
      data: d,
      colors: color,
      resize: !0,
...
$('#class_weekid').change(function(){
    $("#donut-morris-chart-classweek").empty();
    let chart_sbj=$(this).find(':selected').data('id');
    let stdchart_id = $('#profile_username').attr('data-id');
    $.ajax({
      url: '/class_selectweekdata',
      method: 'POST',
