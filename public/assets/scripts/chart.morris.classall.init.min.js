...
let class_allchart = $("#class_allid").val();
let class_stdid = $('#profile_username').attr('data-id');
$.ajax({
  url: '/class_allchart',
  method: 'POST',
  data: {send_chartid:class_allchart, send_id:class_stdid},
  dataType: 'json',
  async: false,
  success:function(data){
    ...
    let calue;
      if(count == '0'){
        $("#all_notification").delay(5).fadeIn('slow').delay(1500).fadeOut('slow');
      }
      else{
  ...
  function initChart_ClassAll() {
    r = Morris.Donut({
      element: "donut-morris-chart-classall",
      data: d,
      colors: color,
      resize: !0,
